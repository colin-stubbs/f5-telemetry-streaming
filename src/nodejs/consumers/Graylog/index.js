...
module.exports = function (context) {
    transformData(context)
        ...
        .catch((err) => {
            context.logger.exception(`Graylog data processing error: ${err}`, err);
        });
};
...
function safeDataTransform(cb, ctx) {
    return new Promise((resolve) => {
        ...
        ctx.globalCtx.logger.exception(`Graylog.computeSourceType error: ${err}`, err);
    });
}
/**
...
function forwardData(dataToSend, globalCtx) {
    if (!dataToSend || dataToSend.length === 0) {
        globalCtx.logger.debug('No data to forward to Graylog');
